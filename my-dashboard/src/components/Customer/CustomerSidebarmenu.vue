...
// ── FIX: updateFilters now always passes current expandedCategory ──
const updateFilters = (sellerId: string = '') => {
  ...
  emit('update:filters', {
    ...
    category: expandedCategory.value,   // ← THIS was the bug — category was not being sent
    seller:   sellerId,
...
// ── When category changes, re-emit filters (seller already selected) ──
const selectCategory = (catValue: string) => {
  // Toggle category expand
  expandedCategory.value = expandedCategory.value === catValue ? '' : catValue

  // If a seller is already selected, re-fetch with new category
  ...
  >
    ...
    <div class="p-4 border-t border-gray-100 bg-gray-50/30">
      <Button
        ...
        class="w-full flex justify-center hover:text-red-600"
